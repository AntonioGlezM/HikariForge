...
| 25 jun 2026 | Frontend | El contenedor principal de la web (.hf-wrap) se ensancha de 1160px a 1600px (y el mega-menú a juego), para aprovechar las pantallas grandes y evitar los huecos laterales, al estilo de tiendas como WLMOUSE. La parrilla de productos, al usar auto-fill, muestra más columnas automáticamente en pantallas anchas. |
| 25 jun 2026 | Frontend | El contenedor principal pasa de un ancho fijo a uno responsivo (width:90% con tope max-width:1800px) para que la web aproveche bien cualquier pantalla sin grandes huecos laterales: en portátiles y monitores normales ocupa el 90%, y solo se topa en 1800px en monitores muy grandes para no perjudicar la legibilidad del texto. Mismo criterio en el mega-menú. |
| 25 jun 2026 | Frontend | Footer rediseñado al estilo de tiendas de periféricos (tipo WLMOUSE): se añade una franja de ventajas/garantías arriba (atención al cliente, envío rápido, garantía, pago seguro con iconos), el cuerpo del footer pasa a ser oscuro fijo en ambos temas (claro/oscuro), con cinco columnas (marca + redes sociales, Productos, Soporte, Explora, Síguenos) y barra inferior con copyright y métodos de pago. Responsive: 4→2→1 columnas según el ancho. |
| 25 jun 2026 | Frontend | Se adopta la paleta de color definitiva de la marca: azul-violeta #4e48e0 (derivado del logo elegido) como acento, sustituyendo al violeta de prueba en ambos temas (claro y oscuro), con sus tonos deep/soft recalculados. Se integran los logos en su versión azul (isotipo, logo horizontal compuesto y favicon). Se genera un backup del estado anterior (paleta violeta). |
| 25 jun 2026 | Frontend | Cambio de identidad a paleta MONOCROMA: el acento deja de ser un color y pasa a negro #141414 en tema claro y blanco hielo #eef0f5 en tema oscuro (todo lo que era azul/violeta —botones, precios, badges, enlaces— ahora es negro/blanco). El logo es adaptativo: versión en tonos de negro para el tema claro y en tonos de blanco hielo para el oscuro (navbar elige según el tema; el footer, al ser siempre oscuro, usa siempre la versión hielo). La barra del navbar se hace más alta (padding 14→22px) y el logo más grande (50→64px). El resalte del hero pasa a blanco hielo para contrastar sobre el fondo oscuro. |
| 25 jun 2026 | Frontend | Se adapta la barra de anuncios superior (AnnounceBar: iconos sociales + marquesina) a la paleta monocroma. Tenía colores violeta fijos (fondo, iconos y texto) que no cambiaban con el tema; ahora usa las variables de la paleta (fondo oscuro neutro, texto gris claro, hover en blanco hielo). Verificado que no quedan colores violeta/azul hardcodeados en el resto del CSS ni en estilos inline. |
| 25 jun 2026 | Frontend | El botón de finalizar compra (checkout) muestra un spinner y queda desactivado mientras se procesa el pedido, evitando dobles envíos en el momento más crítico de la tienda. |
| 25 jun 2026 | Frontend | Vaciar el carrito pide confirmación en dos pasos (Cancelar / Sí, vaciar) en lugar de borrar de golpe, para prevenir borrados accidentales. |
| 25 jun 2026 | Frontend | Nuevo sistema de avisos breves reutilizable (ToastContext): al añadir o quitar de favoritos aparece un mensaje que se desvanece solo. Montado como proveedor global. |
| 25 jun 2026 | Frontend | El catálogo muestra un estado de error de red amable (icono + mensaje + botón Reintentar) si la API no responde, en lugar de quedarse sin contenido. |
| 25 jun 2026 | Frontend | Corrección de contraste en modo oscuro: con la paleta monocroma el acento es blanco hielo, pero 16 elementos tenían el texto/icono fijo en blanco (#fff) encima, quedando ilegibles (botón del chatbot, icono y botón de Discord, botones principales, badges, chips de filtro, pestañas de admin, etc.). Se añade la variable --on-accent (blanco en claro, negro en oscuro) y se aplica en todos esos elementos para que el texto sobre el acento siempre contraste. |
| 25 jun 2026 | Frontend | Menú de navegación móvil: en pantallas pequeñas (donde los enlaces del navbar se ocultaban sin alternativa, dejando al usuario sin navegación) aparece un botón hamburguesa que abre un panel lateral con las categorías, el acceso al catálogo completo, los enlaces de soporte y los accesos de cuenta (perfil/pedidos/favoritos o entrar). Nuevo componente MobileMenu, coherente con el estilo de los drawers existentes y la paleta monocroma. |
